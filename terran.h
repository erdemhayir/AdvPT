...
public:
	...
	map<string, vector<int>> workerIDsMap;		// have to distinguish between initial workers and other workers added later during simulation, key = name, value = ID

	int buildDetails[34][10];			// All the details of items, read from .txt file
	int initWorkerIDs[6][2];			// element number -> ID of initial worker, contained values which specify items workers are working at

	vector<vector<int>> beingCreated;	// 2 columns : 1st column -> mapNumber, 2nd column -> time remained
	vector<vector<int>>	workerIDs;		// row number -> ID of worker, contained values which specify items workers are working at 
	...
	vector<vector<double>> specialbeingCreated;
	// ---------- FUNCTIONS ------------- //
	Terran();
	void initMap();
